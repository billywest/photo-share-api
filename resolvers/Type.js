...
module.exports = {
  Photo: {
    ...
    taggedUsers: async (parent, args, { db }) => {
      ...
      const logins = tags.filter((t) => t.photoID === parent._id.toString()).map((t) => t.githubLogin);
      return db
        .collection('users')
  ...
  User: {
    ...
    inPhotos: async (parent, args, { db }) => {
      const tags = await db.collection('tags').find().toArray();
      const photoIDs = tags.filter((t) => t.githubLogin === parent.githubLogin).map((t) => ObjectID(t.photoID));
